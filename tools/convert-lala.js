/**
 * Convert the Lala Hardeval AI detection workbook into the replay JSON the app plays.
 *
 ...
 * has a broken expat. Run:  node tools/convert-lala.js
 */
const fs = require('fs');
...
const XLSX =
  process.argv[3] || '/Users/wageshsharma/Downloads/Lala_Hardeval_AI_Detection_Report.xlsx';
...
const CLIPS = [
  {
    clip: 'Clip 1',
    out: 'lalaHardevalClip1Replay.json',
    events: [
      // 3.07s Bare Ground cond 14.9 · 0.00s Ground Litter cond 41.4
      // 24.73s Water Body cond 24.9 · 0.13s Shrub/Hedge cond 39.4 · 67.07s Walking Track cond 33.1
      { time: 5.0, type: 'warning', title: 'Bare patches showing', message: 'Ground cover thinning under the palm avenue — could use re-turfing.' },
      { time: 18.0, type: 'info', title: 'Some litter about', message: 'A bit of loose debris across the lawns — a sweep would tidy it up.' },
      { time: 28.0, type: 'warning', title: 'Water feature needs a look', message: 'Water body looking murky — worth checking circulation and clearing surface waste.' },
      { time: 50.0, type: 'info', title: 'Hedges need a trim', message: 'Shrub line growing uneven in places — trimming would tidy the avenue.' },
      { time: 68.0, type: 'info', title: 'Path surface worn', message: 'Walking track showing wear along this stretch — worth patching.' },
    ],
  },
  {
    clip: 'Clip 2',
    out: 'lalaHardevalClip2Replay.json',
    events: [
      // 0.00s Lawn cond 46.5 · 0.00s Ground Litter cond 35.2
      // 26.30s Tree Canopy cond 59.9 · 39.02s Waterlogging · 42.96s Vehicle in Premises
      { time: 5.0, type: 'info', title: 'Lawn could use attention', message: 'Turf patchy around the formal beds — aerating and overseeding would help.' },
      { time: 16.0, type: 'warning', title: 'Litter building up', message: 'Loose debris along the pathways — this one could use a clean-up.' },
      { time: 27.0, type: 'info', title: 'Canopy looking thin', message: 'Tree canopy a little sparse here — some watering would help.' },
      { time: 40.0, type: 'warning', title: 'Standing water observed', message: 'A little water pooling on the path — worth checking drainage.' },
      { time: 44.0, type: 'info', title: 'Vehicle inside the park', message: 'A vehicle picked up within the boundary — worth confirming it is authorised.' },
    ],
  },
  {
    clip: 'Clip 3',
    out: 'lalaHardevalClip3Replay.json',
    events: [
      // 0.00s Ground Litter cond 49.7 (cleanliness weakest zone) · 9.94s Lawn cond 54.3
      // 0.00s Bare Ground cond 50.5
      { time: 4.0, type: 'warning', title: 'Litter building up', message: 'Loose debris across the parterre — the paving could use a sweep.' },
      { time: 11.0, type: 'info', title: 'Lawn could use attention', message: 'Grass thinning beside the axis — overseeding would bring it back.' },
      { time: 17.0, type: 'info', title: 'A few bare patches', message: 'Some bare ground along the central axis — could do with re-turfing.' },
    ],
  },
];

// ---------------------------------------------------------------- xlsx reader
const TMP = path.join(require('os').tmpdir(), 'lala-xlsx-conv');
