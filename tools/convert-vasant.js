/**
 * Convert the Vasant Udyan AI detection workbook into the replay JSON the app plays.
 *
 ...
 * has a broken expat. Run:  node tools/convert-vasant.js
 */
const fs = require('fs');
...
const XLSX =
  process.argv[3] || '/Users/wageshsharma/Downloads/Vasant_Udyan_AI_Detection_Report.xlsx';
...
const CLIPS = [
  {
    clip: 'Clip 1',
    out: 'vasantUdyanClip1Replay.json',
    events: [
      // 0.00s Ground Litter High cond 40.8 · 0.00s Waterlogging cond 49.2
      // 0.00s Bare Ground cond 51.1 · 15.40s Vehicle in Premises · 22.01s Lawn cond 53.7
      { time: 4.0, type: 'warning', title: 'Litter building up', message: "Loose debris across the play area — this one could use a clean-up." },
      { time: 10.0, type: 'warning', title: 'Standing water observed', message: 'Water pooling near the play area — worth checking the drainage.' },
      { time: 17.0, type: 'info', title: 'Vehicle inside the park', message: 'A vehicle picked up within the boundary — worth confirming it is authorised.' },
      { time: 24.0, type: 'info', title: 'A few bare patches', message: 'Bare, compacted ground under the equipment — could do with re-turfing.' },
      { time: 33.0, type: 'info', title: 'Lawn could use attention', message: 'Grass thinning around the edges — aerating and overseeding would help.' },
    ],
  },
  {
    clip: 'Clip 2',
    out: 'vasantUdyanClip2Replay.json',
    events: [
      // 0.00s Bare Ground High cond 32.9 · 0.00s Walking Track High cond 43.3
      // 0.87s Shrub/Hedge High cond 44.9 · 23.41s Waterlogging cond 46.3
      { time: 4.0, type: 'warning', title: 'Ground cover thinning', message: 'Bare, compacted ground along the path — decompacting and re-turfing would help.' },
      { time: 11.0, type: 'warning', title: 'Path surface worn', message: 'Walking track showing wear along this stretch — worth patching.' },
      { time: 18.0, type: 'info', title: 'Hedges need a trim', message: 'Gaps showing along the shrub line — trimming and gap-filling would tidy it.' },
      { time: 25.0, type: 'warning', title: 'Standing water observed', message: 'A little water pooling beside the tree belt — worth checking drainage.' },
    ],
  },
];

// ---------------------------------------------------------------- xlsx reader
const TMP = path.join(require('os').tmpdir(), 'vasant-xlsx-conv');
