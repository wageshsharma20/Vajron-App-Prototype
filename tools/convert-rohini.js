/**
 * Convert the DDA Park Rohini AI detection workbook into replay JSON the app plays.
 *
 * This workbook follows the same layout as Vasant Vatika: a multi-clip survey
 * (Clip / Zone columns) with the construction-aware Frame Timeline columns.
 * Columns are resolved by header name rather than by fixed index.
 *
 * Each clip gets its own dataset JSON with its clock starting at zero. The app
 * chains the three clips in order so they play as one continuous survey.
 *
 * Run:  node tools/convert-rohini.js
 */
const fs = require('fs');
...
const XLSX =
  process.argv[3] || '/Users/wageshsharma/Downloads/Rohini_AI_Detection_Report.xlsx';
const OUT_DIR = process.argv[2] || path.join(__dirname, '..', 'src', 'data');

/**
 * One dataset per clip. Events are in-app alerts anchored to real anomalies in
 * the register, fired a beat after first-seen so the feature is on screen when
 * the banner appears.
 */
const CLIPS = [
  {
    clip: 'Clip 1',
    out: 'rohiniDdaClip1Replay.json',
    events: [
      // Clip 1: Lawns, gazebo & hedge beds — 36.88s
      { time: 5, type: 'info', title: 'Hedges need a trim', message: 'Hedge beds looking uneven along the edges — a trim would tidy things up.' },
      { time: 15, type: 'warning', title: 'Bare patches on the lawn', message: 'Patches of bare ground showing through the lawn — needs replanting.' },
      { time: 28, type: 'info', title: 'Gazebo area looks tidy', message: 'Gazebo structure and surroundings in decent shape — minor upkeep only.' },
    ],
  },
  {
    clip: 'Clip 2',
    out: 'rohiniDdaClip2Replay.json',
    events: [
      // Clip 2: Lake - open water — 26.18s
      // Anomaly: Floating Waste Critical cond 10 @ 22.11s
      { time: 4, type: 'warning', title: 'Lake water looking murky', message: 'Water quality appears poor — greenish tinge suggests algae buildup.' },
      { time: 15, type: 'info', title: 'Shoreline needs cleanup', message: 'Debris visible along the lake edge — clearing would improve the look.' },
      { time: 23, type: 'error', title: 'Floating waste in the lake', message: 'Waste floating on the surface — needs immediate skimming and cleanup.' },
    ],
  },
  {
    clip: 'Clip 3',
    out: 'rohiniDdaClip3Replay.json',
    events: [
      // Clip 3: Algal channel & banks — 47.85s
      { time: 5, type: 'warning', title: 'Algae spreading in the channel', message: 'Green algae coating the channel surface — water circulation needed.' },
      { time: 18, type: 'warning', title: 'Bank erosion visible', message: 'Channel banks wearing away in spots — could use reinforcement.' },
      { time: 30, type: 'info', title: 'Some litter near the banks', message: 'Loose debris along the channel edge — a cleanup would help.' },
      { time: 42, type: 'warning', title: 'Dry patches along the bank', message: 'Plant cover thinning on the banks — watering and replanting needed.' },
    ],
  },
];

// ---------------------------------------------------------------- xlsx reader
const TMP = path.join(require('os').tmpdir(), 'rohini-xlsx-conv');
...
/** Resolve the first header that exists, or throw. */
...
const D_CODE = col(DLI, ['dtu code', 'code']);
const D_OBJ = col(DLI, ['detected object']);
const D_CAT = col(DLI, ['dtu category', 'category']);
...
// Summary metadata and headline findings describe the whole park.
...
function buildClip(spec) {
  ...
  const anomalies = anomalyRows
    ...
    .map((a) => {
      ...
      return {
        Priority: a['Priority'],
        'Track ID': a['Track ID'],
        Code: a['DTU code'] || a['Code'] || '',
        Anomaly: a['Anomaly'],
        'Sub-parameter': a['DTU sub-parameter'] || a['Sub-parameter'] || '',
        Severity: a['Severity'],
  ...
  // Validate events don't exceed clip duration
...
for (const b of built) {
  console.log(`  ${b.clip}  ${b.zone.padEnd(28)} ${b.end.toFixed(2).padStart(7)}s  ${b.out}`);
  total += b.end;
}
