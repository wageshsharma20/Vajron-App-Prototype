/**
 * Convert the Smriti Van AI detection workbook into the replay JSON the app plays.
 *
 ...
 * has a broken expat. Run:  node tools/convert-smriti.js
 */
const fs = require('fs');
...
const XLSX =
  process.argv[3] || '/Users/wageshsharma/Downloads/Smriti_Van_AI_Detection_Report.xlsx';
...
const CLIPS = [
  {
    clip: 'Clip 1',
    out: 'smritiVanClip1Replay.json',
    events: [
      // 1.87s Water Body High cond 31.1 · 0.00s Ground Litter High cond 38.4
      { time: 3.0, type: 'warning', title: 'Standing water observed', message: 'Water pooling in the planting bed — worth checking the drainage here.' },
      { time: 12.0, type: 'info', title: 'Some litter about', message: 'A bit of loose debris across the bed — a sweep would tidy it up.' },
    ],
  },
  {
    clip: 'Clip 2',
    out: 'smritiVanClip2Replay.json',
    events: [
      // 0.00s Ground Litter cond 20.8 · 15.17s cond 18.5 · 23.00s Waterlogging cond 42.5
      // The play equipment is the one finding kept at full weight — a broken swing
      // in a children's area is a genuine safety issue, not a housekeeping note.
      { time: 4.0, type: 'warning', title: 'Litter building up', message: 'Loose debris across the play area — this one could use a clean-up.' },
      { time: 17.0, type: 'warning', title: 'Path partly blocked', message: 'Some debris sitting on the walking track — worth clearing the route.' },
      { time: 25.0, type: 'warning', title: 'Standing water observed', message: 'Water pooling in the play area — worth checking the drainage.' },
      { time: 36.0, type: 'error', title: 'Play equipment broken — critical', message: 'Swing appears collapsed on the sand. Inspect and make safe before reopening.' },
    ],
  },
  {
    clip: 'Clip 3',
    out: 'smritiVanClip3Replay.json',
    events: [
      // 4.54s Waterlogging cond 49.6 · 7.74s Tree Canopy cond 38.8 · 11.83s Bare Ground cond 29.5
      { time: 6.0, type: 'warning', title: 'Standing water observed', message: 'Water lying across the open ground — worth a look at levels and drainage.' },
      { time: 10.0, type: 'info', title: 'Canopy looking thin', message: 'Tree canopy a little sparse here — some watering would help.' },
      { time: 14.0, type: 'warning', title: 'Ground cover thinning', message: 'Ground is bare and compacted in places — decompacting and re-turfing would help.' },
      { time: 26.0, type: 'info', title: 'Some litter about', message: 'A bit of loose debris across the open ground — worth a sweep.' },
    ],
  },
  {
    clip: 'Clip 4',
    out: 'smritiVanClip4Replay.json',
    events: [
      // 2.80s Bare Ground cond 42.4 · 5.94s Waterlogging cond 46.2
      // 20.59s Lawn cond 44.8 · 29.97s Shrub/Hedge cond 29.5
      { time: 4.0, type: 'info', title: 'A few bare patches', message: 'Some compacted ground along the path — could do with re-turfing.' },
      { time: 7.0, type: 'warning', title: 'Standing water observed', message: 'A little water pooling beside the tree belt — worth checking drainage.' },
      { time: 21.0, type: 'info', title: 'Lawn could use attention', message: 'Grass thinning in patches — aerating and overseeding would bring it back.' },
      { time: 31.0, type: 'info', title: 'Hedges need a trim', message: 'A few gaps showing along the shrub line — trimming and gap-filling would tidy it.' },
    ],
  },
];

// ---------------------------------------------------------------- xlsx reader
const TMP = path.join(require('os').tmpdir(), 'smriti-xlsx-conv');
